...
Unlike Kullback-Leibler distance, the mutual information is symmetric. It follows that the amount of information $X$ has about $Y$ is always equal to the amount of information $Y$ has about $X$. 
...
From these equations, it can be seen that finding the partial information decomposition terms amount to solving various optimization problems. It is important to note, however, that only one of the these optimization problems need to be solved in practice. Once one of the PID terms is found, the system of equations (\ref{eq:pid}, \ref{eq:pid-yz}) becomes determined, being composed of 3 equations and 3 unknowns, and as such, the other PID components can be calculated with basic algebra.

\textcolor{red}{TODO! Get the probability distribution notations straight!}
...
\textcolor{red}{Consult Dirk on the estimator?}
...
\addcontentsline{toc}{section}{Appendix A: Elementary Cellular Automata}

\newpage
\section*{Appendix B: Feedforward Neural Networks}
\addcontentsline{toc}{section}{Appendix B: Feedforward Neural Networks}
